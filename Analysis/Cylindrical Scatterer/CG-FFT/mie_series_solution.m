...
for counter = 1:problem_size
   ...
   if (k == k0)
      ...
      % NB: k below should be kd... 
      for n = low_limit:high_limit
         An_top = (eta0 / etad) * besselj(n, k0*radius) * 0.5 * (besselj(n-1, kd*radius) - besselj(n+1, kd*radius)) - 0.5 * (besselj(n-1, k0*radius) - besselj(n+1, k0*radius)) * besselj(n, kd*radius);
         
         An_bottom = besselj(n, kd*radius) * 0.5 * (besselh(n-1, 2, k0*radius) - besselh(n+1, 2, k0*radius)) - (eta0 / etad) * 0.5 * (besselj(n-1, kd*radius) - besselj(n+1, kd*radius)) * besselh(n, 2, k0*radius);
      ...
      for n = low_limit:high_limit
          ...
          Bn_bottom = (pi*k0*radius) * (besselj(n, kd*radius) * 0.5 * (besselh(n-1, 2, k0*radius) - besselh(n+1, 2, k0*radius)) - (eta0 / etad) * 0.5 * (besselj(n-1, kd*radius) - besselj(n+1, kd*radius)) * besselh(n, 2, k0*radius));
          
          Bn = Bn_top / Bn_bottom;
          
          E_total_mie_sum = E_total_mie_sum + power(1i,-n) * Bn * besselj(n, kd*rho(counter)) * exp(1i*n*phi(counter));
