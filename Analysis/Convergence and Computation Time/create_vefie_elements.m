...
for counter = 1:problem_size
    ...
    %R = abs(position(counter, 1) - start_point);
    ...
    for row = 1:problem_size
        ...
        if (row == counter) % M = N => Self term
            ...
            %G(row, counter) = (1i / 4) * (leading_G_factor * (besselj(1,k0*equiv_a)  - 1i*bessely(1,k0*equiv_a)) - 4.0*1i / (k0*k0));
            ...
            %G(row, counter) = (1i / 4) * (leading_G_factor * besselj(1, k0 * equiv_a) * (besselj(0,k0*Rmn) - 1i*bessely(0,k0*Rmn)));
        end
